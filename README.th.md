...
<br><sub>³ EV-008 — bare เทียบกับ **overflow sentinel** ซึ่งเป็นฟีเจอร์ที่ยังอยู่ระหว่างออกแบบ ([#159](https://github.com/caty-ai/caty-agent-harness/issues/159)) และ **ยังไม่เผยแพร่** เป็นการวัดบน rig ก่อนการพัฒนา `ไม่ลดลง` = ทั้งสองฝั่งได้ 20/20 ตามเฉลยลับในทุกเซลล์ `token เทียบกับ bare` = 1 − median(sentinel/bare) จาก 4 เซลล์ที่ปิดผนึก: sonnet **0.801** → −20%, opus **0.923** → −8% `—` = ไม่ได้วัดในเลนนี้</sub>
