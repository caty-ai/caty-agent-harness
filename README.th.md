...
- **การหลอกว่า "เสร็จ" หยุดทำงานบนโมเดลที่อ่อนกว่า — งานเสร็จจริง** บน Claude Haiku 4.5 (โมเดลที่มีความสามารถต่ำกว่า) ที่เคยอ้างว่า "เสร็จ" ทั้งที่ยังไม่ได้ทำงานถึง 98% ของกรณี การอ้างเท็จลดลงเหลือ 8% — และอัตราการทำงานสำเร็จที่ยืนยันแล้วเพิ่มขึ้นจาก 13% เป็น 43%
- **โมเดลที่แข็งแกร่งยังคงความแม่นยำ และใช้ token น้อยลงในงานใหญ่** sonnet และ opus ได้คะแนนเท่ากันทั้งแบบมีและไม่มี harness — แต่ในงานขนาดใหญ่ การรันโดยไม่มี harness ใช้ token เป็น 2.40 เท่า (sonnet) / 1.46 เท่า (opus) ของแบบมี harness ในงานขนาดเล็กของ sonnet ในการทดสอบชุดนี้ harness กลับใช้ token มากกว่า ไม่ใช่น้อยกว่า — ควรใช้ในงานที่ใหญ่จริง ๆ และในงานที่ใหญ่เกิน context window ประมาณ 2.4 เท่า ซึ่ง opus แบบไม่มี harness อ่านไฟล์ได้แค่ 2–4% แล้วยังพยายามส่งงานต่อไป harness กลับทำงานสำเร็จแบบยืนยันแล้ว — อ่านไฟล์ครบ 100% — ในทุกเซลล์ที่วัดผล ([2/2 ในทั้งสองช่วง overflow](https://github.com/caty-ai/caty-agent-harness/issues/235); ทั้งสองฝั่งรันภายใต้ขอบเขตงบประมาณที่ไม่เท่ากัน)
- **ผลลัพธ์ยังแสดงข้อจำกัดของมันด้วย** รันไทม์แบบ search (เช่น Codex) ไม่ได้ประโยชน์อะไรเลย — ซึ่งเป็นไปตามการออกแบบ — และมีรันไทม์หนึ่งที่ทำงานแต่ต้นทุนสูงกว่าการรันโดยไม่มี harness sonnet ในช่วง overflow ยังคงถูกต้อง 20/20 แต่ข้ามไฟล์ไปประมาณ 10–12% — เกตการตรวจสอบปฏิเสธคำว่า "เสร็จ" ทุกครั้ง พร้อมระบุรายชื่อไฟล์ที่ยังไม่ได้อ่าน ทุกผลลัพธ์ที่แพ้ถูกเผยแพร่คู่กับผลลัพธ์ที่ชนะ

<sub>การทดลองแบบปิดผนึก ลงทะเบียนล่วงหน้า และให้คะแนนโดยเครื่อง 4 ชุด (2026-08) — ตัวเลข วิธีการ และข้อควรระวังทั้งหมด รวมถึงจุดที่ harness มีต้นทุนเพิ่มขึ้น: [benchmark](docs/benchmark.md) ・ **ลองใช้ได้ใน 2 นาที** — วาง[พรอมป์ต์เดียวที่คัดลอกมาได้เลย](#get-started) ลงในเครื่องมือ AI ที่คุณใช้อยู่แล้ว</sub>
