...
**overflow sentinel** ([design issue #159](https://github.com/caty-ai/caty-agent-harness/issues/159)) เฝ้าดูระดับ context ต่อเทิร์นที่วัดได้จริง และเมื่อเกินเกณฑ์ จะหยุดการรันแล้วแตกงานออกเป็นส่วนย่อย แทนที่จะปล่อยให้ context ล้น EV-008 — เบนช์มาร์กแบบปิดผนึกและลงทะเบียนล่วงหน้า (2026-08) — วัดพฤติกรรมนี้แยกตามโมเดล ผลลัพธ์แบ่งตามวิธีที่รันไทม์ "อ่าน" context:

| ประเภทรันไทม์ | โมเดลที่วัดผล | พฤติกรรม | ข้อสรุป |
|---|---|---|---|
| **Full-read** — context เพิ่มขึ้นแบบ monotonic | claude-haiku-4.5 · claude-sonnet-5 · claude-opus-5 | ทำงาน (fire) กลางงาน → แตกงาน → ทำสำเร็จ; ความถูกต้องรักษาที่ 20/20 ในทุกเซลล์ที่ทำงาน | **นี่คือจุดที่ได้ประโยชน์จริง** ค่ามัธยฐาน sentinel/bare ของ sonnet คือ **0.801** (เซลล์ที่ดีที่สุด token ลด −71%) ・ opus **0.923** ・ haiku 0.35 (descriptive lane) |
| **Search-type** — context นิ่งอยู่ใต้เกณฑ์ | gpt-5.6-luna (Codex) · qwen3.8-max | ไม่ทำงาน (fire) เลย: codex **0/127 turns** ・ qwen 0/4 เซลล์ (ค่าสูงสุดที่วัดได้ 79.7K จากเกณฑ์ 80K) | **การไม่ทำงานคือสิ่งที่ออกแบบไว้** — นี่คือเงื่อนไขความปลอดภัยของ default-on เอง หากทำงานที่นี่จะถือเป็น false positive และต้องส่งกลับไปแก้การออกแบบ |
