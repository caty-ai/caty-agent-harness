...
**วัดผลแล้ว** — การทดลองแบบปิดผนึกและลงทะเบียนล่วงหน้า 3 ชุด (2026-08) ตารางแสดงสองเลนของการแทรกแซงบนงานที่ context ล้น ส่วนชุดที่สาม (P2-WIN⁵) คือบันไดของงานที่รันบนโมเดลเปล่า ไล่ระดับตั้งแต่อยู่ภายใน context window อยู่ที่ขอบ ไปจนถึงเกินออกจาก context window:

| โมเดล | อาการหลอนว่าเสร็จแล้ว¹ | ผลลัพธ์ที่ยืนยันแล้ว | token เทียบกับ bare |
|---|---|---|---|
| claude-haiku-4.5 ² | 98% → 8% | 13% → 43% (p=0.0079) | −59% |
| claude-sonnet-5 ³ | — | ไม่ลดลง | −20% (แต่ละเซลล์ −71%…+11%) |
| claude-opus-5 ³ ⁴ | — | ไม่ลดลง | −8% |

<sub>¹ อ้างว่า "เสร็จ" ทั้งที่ยังไม่ได้อ่านงานที่ทำ — ให้คะแนนโดยเครื่องจาก transcript การเรียกใช้ tool ไม่ใช่จากการรายงานตัวเอง (222 → 2 คำอ้าง, 30 รัน/ฝั่ง): [ตัวเลขและข้อจำกัดฉบับเต็ม (ภาษาอังกฤษ)](docs/benchmark.md)</sub>
<br><sub>² EV-006 — bare เทียบกับ **harness ที่เผยแพร่แล้ว**; อัตราทำงานสำเร็จที่ยืนยันแล้วจาก 30 รัน/ฝั่ง (ขนาด M/L)</sub>
<br><sub>³ EV-008 — bare เทียบกับ **overflow sentinel** — Sentinel v1 เผยแพร่แล้วแบบ opt-in สำหรับ claude-code runtime ใน v0.17.0 ([#180](https://github.com/caty-ai/caty-agent-harness/issues/180) เป็นการ implement [#159](https://github.com/caty-ai/caty-agent-harness/issues/159)) เปิดใช้ด้วย [`OVF_SENTINEL=shadow|active`](adapters/claude-code/INSTALL.md) และถ้าไม่ตั้งค่า = ปิดเต็มรูปแบบ (byte-identical passthrough) default-on ยังเป็นงานในอนาคต โดยตัดสินแยกตามโมเดลตามเงื่อนไขของ [#159](https://github.com/caty-ai/caty-agent-harness/issues/159) ดู [โปรไฟล์รายโมเดล](#model-effects) EV-008 ยังคงเป็นการวัดล่วงหน้าบน rig ก่อน implementation นี้ และยังไม่ได้วัดซ้ำบน implementation ที่เผยแพร่แล้ว `ไม่ลดลง` = ทั้งสองฝั่งได้ 20/20 ตามเฉลยลับในทุกเซลล์ `token เทียบกับ bare` = 1 − median(sentinel/bare) จาก 4 เซลล์ที่ปิดผนึก: sonnet **0.801** → −20%, opus **0.923** → −8% `—` = ไม่ได้วัดในเลนนี้</sub>
<br><sub>⁴ เชิงพรรณนา, post-GO</sub>
<br><sub>⁵ P2-WIN — bare model เท่านั้น ไม่มี harness ไม่มี sentinel: งานต้องใหญ่แค่ไหนถึงจะรันไม่จบ? บนงานขนาด ≈0.6M / ≈1.2M / ≈2.4M token, sonnet เริ่มรันไม่จบตั้งแต่ระดับ ≈1.2M ขึ้นไปในทั้งสอง hold-out seed (transfer band = XXL — ระดับที่เล็กที่สุดที่มีอัตราสำเร็จ 0/2 ตามที่ตารางนั้นบันทึกไว้); สอง hold-out seed ของ opus ตัดสินไม่ตรงกันในระดับหนึ่ง (1/2 — ดูตารางว่าระดับไหน) → **no unique transfer** (กฎที่ปิดผนึกไว้ไม่ระบุระดับใดเป็นการเฉพาะ) และใน ticket ขนาด 2.4M ของ opus ยังคงทำคะแนนได้ 19–20/20 ต่อเนื่อง ในขณะที่ attempt-1 file-count read coverage (coverage_001) ตกลงไปเหลือ 0.02–0.04 n=2 seed ต่อเซลล์ เป็นเพียงข้อมูลเชิงพรรณนา — ไม่ใช่การอ้างผลของการแทรกแซง: [ตารางและข้อจำกัด (ภาษาอังกฤษ)](docs/benchmark.md#p2-win)</sub>

<sub>Codex, qwen, grok, gemini และรันไทม์ที่มองไม่เห็น telemetry (glm/muse/kimi) ก็ถูกวัดแล้ว รวมถึงเซลล์ที่ sentinel ใช้ต้นทุนสูงกว่า bare: [โปรไฟล์รายโมเดล](#model-effects) ・ [ตัวเลขและประวัติฉบับเต็ม (ภาษาอังกฤษ)](docs/benchmark.md#ev-008) ・ เลนที่วางแผนไว้รวมโมเดลภายในเครื่อง (Ollama): [#129](https://github.com/caty-ai/caty-agent-harness/issues/129)</sub>
