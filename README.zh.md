...
**它会把大任务拆成阶段，确认工作真的做完了——并且把自己的成绩单公开发布，包括失败的部分。**

**实际会变好的地方** — 是实测出来的，不是承诺：

- **在弱模型上，虚假的「完成」不再蒙混过关——任务真的能做完。** 在 Claude Haiku 4.5（能力较弱的模型）上，原本 98% 的情况下会在没做完工作时就宣称「完成」，如今虚假宣称降到了 8%——已验证的完成率也从 13% 升到了 43%。
- **强模型保持准确率，在大任务上用更少的 token。** sonnet 和 opus 在有无 harness 的情况下得分相同——但在大任务上，不用 harness 时用掉的 token 是有 harness 时的 2.40 倍（sonnet）/ 1.46 倍（opus）。在这些测试里，sonnet 的小任务用 harness 反而更费 token——用在大任务上才划算。而在超出上下文窗口约 2.4 倍的任务上，没有 harness 的 opus 只读了 2–4% 的文件就不断尝试提交，harness 则在每个测量单元里都做到了验证通过——100% 文件读取完成（[两个溢出档位均为 2/2](https://github.com/caty-ai/caty-agent-harness/issues/235)；两组测试使用的预算范围并不相同）。
- **结果也展示了它的局限。** 搜索型运行时（例如 Codex）没有获得任何收益——这是设计使然——还有一个运行时虽然触发了，但成本反而比不用 harness 更高。sonnet 在溢出档位里保持 20/20 正确，但会漏读约 10–12% 的文件——验证关卡每次都拒绝了「完成」，并列出未读的文件。每一个失败的结果，都和成功的结果一起公开。

<sub>四组密封、预先注册、机器评分的实验系列（2026-08） — 全部数字、方法与注意事项，包括 harness 反而更费成本的地方：[benchmark](docs/benchmark.md) ・ **2 分钟即可上手** — 把[这一段可直接粘贴的提示词](#get-started)，贴进你已经在用的 AI 工具。</sub>
...
<sub>比值为 sentinel/bare 的 token 成本（数值越低越省）・每个模型 4 个密封单元的中位数・实测于 2026-08-25。引用前请先读：① codex 条件最初 **FAIL**（M4，n=1，几何平均 1.337），只有在 n=3 重复测试——这是看到数据后才制定、并经 3 席 delta 审核封印的事后设计（0.9944 ≤ 1.05）——中才通过；FAIL 是历史，不会被抹去；② sonnet 的 0.801 达到了判定阈值（<1.0），但以微弱差距未达到努力目标（<0.8）；③ 大多数逐对比值都是 n=1——run 与 run 之间的方差是真实存在的（codex 平均值的 SE ≈25%）；n=3 重复补充测试（2026-08-29）对此做了量化——每个模型汇总 12 个对数比后：sonnet 几何平均 **0.617** [0.439, 0.867]，opus **0.806** [0.651, **0.997**]——单次测量的中位数都落在这个区间之内（各次重复范围为 0.28–1.50），opus 的置信上限是 0.997：省下的成本得到确认，但差距非常小。[完整数字、方法与注意事项（英文）](docs/benchmark.md#ev-008) ・ [n=3 补充测试（英文）](docs/benchmark.md#ev-008-n3)。</sub>
