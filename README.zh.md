...
另一条实验线 **EV-007 / EV-007b** 测量了主张的另一半——学习回路。结果是**两次都没有学习事件**。EV-007 在跨模型评审 fail-close 后终止；随后 EV-007b 构建了一套注入陷阱（trap）的测量工具，它确实会让错误重复出现（在学习臂上，v0.24.0 每一轮都有 8/8 个错误类别复现），并在其上运行了封存后的正式运行——但评审拒绝了其中的每一条引用，原因是评审的引用校验与 Stop hook 写出的 flush 行格式不匹配，这正是本次运行的产品发现。晋升数：0。因此，这个回路对重复错误究竟有没有作用，**仍然未被测量**——既没有被证明有效，也没有被证明无效。这条线真正产出的是随 v0.24.0 发布的产品改动（晋升的复现单位从 ISO 周改为不同的 session 数）：[停在哪里、为什么（英文）](docs/benchmark.md#ev-007)。
