...
| **触发但结果混合型** — 早期触发・重型全量读取 | gemini-3.7-flash（descriptive——不在预先注册的 GO 条件之内） | 4/4 触发（t31–82）。bare 在 L 档崩溃；分解在一个 L 单元中挽救了完成，但 2/4 的 sentinel 单元正确数为 0（子步骤在 headless 下的 permission 停滞） | **仅作记述——不做效率主张，也不做 default-on 判断。**[单元级数字](docs/benchmark.md#ev-008) |
