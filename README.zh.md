...
|---|---|---|---|
| Claude Haiku 4.5 | 13% → **43%**（+30 pt，p=0.008） | 94% → **5%** | **减半甚至更少** |
| GPT-5.6 Luna（Codex） | 计划中 | — | — |
| 本地模型（Ollama） | 计划中 | — | — |

<sub>¹ 还没读完工作内容就宣称「完成」——依据工具调用记录测量，而非自我报告（247 次 → 2 次）。15 万至 30 万 token 的任务，每组配对运行 30 次，全部由机器评分。包含薄弱环节：[完整数据与局限（英文）](docs/benchmark.md)。</sub>
