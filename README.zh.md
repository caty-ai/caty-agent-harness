...
另一条密封实验线 **EV-007 / EV-007b** 测量了主张的另一半——学习回路。结果是**没有学习事件，也没有正式运行**：跨模型评审两次 fail-close；等到第二套测量工具准备好时，harness + haiku 已经把我们手上的每一个语料集都做到了 19–20/20——已经没有可供回路学习的重复错误样本了。这条线真正产出的是随 v0.24.0 发布的产品改动（晋升的复现单位从 ISO 周改为不同的 session 数）：[停在哪里、为什么（英文）](docs/benchmark.md#ev-007)。
