...
另一条实验线 **EV-007 / EV-007b / EV-007c** 测量了主张的另一半——学习回路，一共有三次封存运行。EV-007 在跨模型评审 fail-close 后终止；随后 EV-007b 构建了一套注入陷阱（trap）的测量工具，它确实会让错误重复出现（在学习臂上，v0.24.0 每一轮都有 8/8 个错误类别复现），并在其上运行了封存后的正式运行——但评审拒绝了其中的每一条引用，原因是评审的引用校验与 Stop hook 写出的 flush 行格式不匹配，这正是那次运行的产品发现；这两条线的晋升数都是 0。**EV-007c 在评审能够正常引用的 v0.25.0 上重跑了同一套封存序列：三个回合全部产生了晋升（晋升 6 项、批准规则 2 条），而封存的通过线没有被满足**——学习臂的错误复现率为 0.667，无学习臂为 0.727，双重差分（DiD）为 **+0.050**。在规则获批后唯一发生移动的那个 T2 类别（文件内状态被覆盖）在第 3 轮被解决，又在最后一个区块丢失了一半；而勘误文件类别在任何臂上都没有移动，也从未被写下来让回路看见。这是在“每个类别只有一个比特”的测量工具上得到的 **n = 1 的一次测量结果**——它是那条预先登记的判定线未达成，而不是对回路本身的裁决。这些实验线真正产出的是随 v0.24.0 与 v0.25.0 发布的两项产品改动：[测到了什么、没测到什么（英文）](docs/benchmark.md#ev-007)。
