...
| `2` | Usage/configuration error in Task Runner (`scripts/task-runner.sh:8-10`; `scripts/task-runner.sh:55-56`), Metrics (`scripts/tr-metrics.sh:8-10`), Deadman Probe (`scripts/deadman-probe.sh:4-6`), Family Updater (`scripts/family-updater:34-44`), Loop Init (`scripts/loop-init:10-26`), Attest Wrapper (`scripts/attest-wrapper:38-71`), and `install.sh`'s `die_usage` path (`install.sh:71-77`). The previously unpinned cases are covered together in `tests/cli-conventions.test.sh:70-75`. |
...
`scripts/task-runner.sh` also treats `TR_SPAWN_STEP` as required spawn-path configuration: it initializes the env var at startup and rejects an empty value with exit 2 (`scripts/task-runner.sh:17`; `scripts/task-runner.sh:55-56`).
