...
The later `umask` lesson is separate. The 2026-08-22 run also showed that a WSL2 result could look "almost supported" while still depending on ambient `umask` behavior. That is why D1' made trust fixtures `umask`-independent and why D2' added the `ubuntu-wsl2-profile` CI cell. If the suite had only passed under `umask 022`, it would still have been too easy to document WSL2 as supported while missing a real-world `umask 002` refusal path. After D1', the raw `umask 0002` run passed 30/30, which is the evidence behind the current wording.
