...
4. In one pull request, update BOTH the `uses:` ref and `generator_ref` in `.github/workflows/repo-state.yml` to the same new peeled commit SHA, retaining the trailing `# vX.Y.Z` tracking comment on both lines. The reusable workflow uses curl to fetch `https://raw.githubusercontent.com/caty-ai/family-os/$GENERATOR_REF/tools/repo-state-gen.sh`; leaving `generator_ref` on a tag would leave the generator script on a mutable ref. The reusable workflow's input description and validation error still say "pinned tag" (`invalid generator_ref; use a pinned tag`); that wording predates SHA pinning, so pass the 40-hex commit SHA regardless and never revert `generator_ref` to a tag.
5. Because the pull request touches `.github/workflows/*`, `risk-review-gate` remains red until a roster human applies `risk-reviewed`. That red is a process gate, not evidence of broken workflow code. The pull request's own `repo-state / check repository state contract` job already resolves the reusable workflow and fetches the generator at the new SHA, so a wrong pin fails before merge.
