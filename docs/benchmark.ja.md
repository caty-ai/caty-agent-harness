...
ハーネスはほぼ常に全部読んでから申告します — ゲート却下された納品19件のうち
未読ファイルを含んだのは2件で、その2件もゲートが捕捉しました — そして前に
...
溢れサイズの納品の半分は外部ゲートに却下されています。消えるのは**未読の
申告** — 読んでいないのに「done」と言うこと、です。
...
- 全実行は作者管理下の環境です。一時的な CLI 障害バーストが2回あり、
  影響 sequence は事前登録済みの手続きで再走しました。うち1本は再走を
