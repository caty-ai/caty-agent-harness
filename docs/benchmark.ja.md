...
5. **EV-007 / EV-007b — 学習ループ。**「レビューしてから昇格」は同じ間違いの
   ...
   fail-close し、次の計測器を用意した頃にはハーネスが手持ちのコーパスを
   すべて解いてしまっていました。レーンが残したのは製品変更と実地の配線証明
...
です: 2026-08-19 のピンではハーネスは `p3-M` コーパスで1ジョブあたり 13.6
件の誤答を出していましたが、ここで使ったピンでは同じコーパスが 20/20 です。
...
| 段 | コーパス | 姿勢 | 正答（2ジョブ） | ジョブ別 mistake | charge |
|---|---|---|---|---|---|
...
ではなく（判定 `neither`）、retry ループでもありません（4本のうち2本は計画
段数ちょうどで delivered・再試行なし・間違いもなし）。

データが許すかぎり狭く述べた発見: **v0.24.0 のハーネス + haiku は、
EV-006/EV-009 ファミリーのあらゆるコーパスを 19〜20/20 で解く（約 2.4M ト
ークンのコーパスまで）— この主張が必要とする「繰り返す間違い」の母集団は、
既存のどの計測器にも存在しない。** オーナーの停止規則に従い、封印もせず本
...
`unit=sessions` — その中には、2つのメンバーが**同じ日**のファイル内にあり
ながら `session=` が2つ異なるテーマ（`run-k: 2`・週数は 1）が含まれます。
...
`skipped-already-applied=16`（冪等）。STATE.md に入った 14 行はすべて完全
な出所トレーラ（
...
2. **計測器が死んだのは製品が良くなったからです。** 2026-08-19 のピンでは
   ハーネスは `p3-M` で1ジョブ平均 13.6 件（`p3-L` で 17.6 件）の誤答を
   出していました。v0.21.1 / v0.24.0 では同じコーパスが 19〜20/20 です。
...
5. **配線再確認から出た製品への3つの観察** — 主張ではなく、Issue へのポイ
   ...
   視します** — 該当4テーマが昇格しました。つまり審査側は昇格を拒否でき
   ません（
...
度ラダーに置き換えられました**（#264 に記録・2026-09-03 に計測器選択（案1・不合格なら停止）と併せてオーナーが承認）。ラダー r2
...
| **EV-007b 合計・80M 上限の枠外として報告** | **≈176.7M** |
...
はどちらの読みでも上限を超えていました。ラダー水準ごとの1ジョブあたり
charge は上の EV-007b の表にあります — 高いのは溢れ帯の2水準です（XXL 1ジ
