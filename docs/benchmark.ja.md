...
5. **EV-007 / EV-007b — 学習ループ。**「レビューしてから昇格」は同じ間違いの
   再発を止めるか。2レーン・封印済み本走2本、**どちらも学習効果の数字は出て
   いません**: EV-007 は審査が2回 fail-close して終了。EV-007b は「間違いが
   再発する」計測器を trap 注入で作り直し（学習側の腕で毎ラウンド 8/8 クラスが再発）、
   それを使って封印済みの本走を実行しました — が、その本走でも3ターンすべて
   の審査が fail-close し、昇格は1件も起きませんでした。レーンが残したのは
   製品変更・実地の配線証明・そして動く計測器です — [EV-007](#ev-007) を参照。

ここにある本走はすべて、開始前に封印・事前登録されています（EV-007b の本走も含む —
2026-09-04 封印・pin `v0.24.0`）。較正パイロットと計測器探索は事前に書かれた規則の
もとで封印前に走り、封印後の訂正3件は上書き記録として台帳に残しています。すべて機械
採点され、限界の記述つきで報告されています — ハーネスが勝てなかった場所も含めて。
...
していません: EV-007 は**学習イベント 0**で終了し、EV-007b は — 事前登録を
封印し、そのために作った trap 注入型の計測器の上で本走も実行したうえで —
3つのループターンすべてが審査段階で fail-close したため、同じ終端結果
`unmeasurable-no-learning-event` に至りました。この節は両方の停止・原因・
最終的に動いた計測器・レーンが生んだ製品変更を公開します。このページの規律
は同じです — 見出しを出せなかった走行も書き残します。
...
を引用できなかったのです。だからループは何ひとつ昇格させず、測るものが生まれません
でした。
...
1. **これは学習についての「効果なし」の結果ではありません。** 2レーン通し
   て昇格 0・承認ルール 0。EV-007 には after ブロックがそもそも存在せず、
   EV-007b には存在しますが、3つのループターンで学習イベントが起きなかっ
   たため、封印済みの帰属規則により C 対 B の対比は帰属不能です。「レビュ
   ーしてから昇格」というパイプラインの命題は、ここでは**反証されたので
   はなく、未検証**です。この節から「ループは効く」も「ループは無駄」も読
   み取らないでください — どちらも支持されていません。
...
7. **計測器のギャップ。** ハーネス腕の `telemetry.json` は EV-007 の全 16
   ...
   ありました。
8. **EV-007b の計測器はこのピンで機能します。** それ以前のどのインスタンス
...
9. **見出しの DiD は結果ではありません。** `+0.400` と 1.000 / 0.643 の再
...
10. **審査の引用契約と Stop hook の flush 形式が v0.24.0 では噛み合ってい
    ...
    きる審査だったら何かを昇格させたかどうかは、**この走行からは判定でき
    ません**。EV-007 の「審査のばらつきが fail-close の境界に乗っていた」
    という観察に、今回は機構が付きました。
...
13. **本走の姿勢と限界。** 18 ジョブすべてが D4 正規化済みスロットで走りま
...
以下はどれも予定に入っておらず、日付の約束もしません。計測器の問題は解決しました:
`p3trap-M-i1` は存在し、封印されており、`v0.24.0` の上で間違いを再発させます —
erratum ファイル型の 5 クラスは採点済みハーネス 14 ジョブすべてと素の2ジョブで
mistake になり、学習側の腕は毎ラウンド 8 つの trap クラス全部を出しました。
残っているのは審査段階です。

- **審査が、渡された生ログ層を引用できるようにしたうえで、同じ封印済み系列を再走行
  する。** 製品側の修正候補は2つ、コミットではなくポインタとして挙げておきます:
  `raw-review.sh` の引用チェックの正規化で Stop hook の `(日付, task-id)` 接頭辞も
  剥がすか、200 文字という引用長の上限を上げるか。どちらかが入るまで、このピンで
  同日のループターンは candidate を1つも作れず、見出しは未測定のままです。
- **Sonnet/Opus ラダー**
...
- **確証的な主張の前に hold-out インスタンスを。** `p3trap-M-i2`（同じ trap 構成で
  語彙だけ差し替えたもの）は封印時に限界として名指ししたまま、まだ作られていません。
  n = 1 走行・1クラスあたり1ビット・hold-out なし — ここにあるものを、それ抜きで確証
  的な結果に変えてはいけません。
