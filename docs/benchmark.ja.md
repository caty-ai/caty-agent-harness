...
ここにある走行はすべて、開始前に封印・事前登録され（EV-007b の本走は開始されず、
事前登録は草稿のままです）、機械採点され、限界の記述つきで報告されています — ハーネスが勝てなかった場所も含めて。
...
j4〜j7 は 20/20）が、P0.7 では同じ `p3-M-i1` が4ジョブすべてで 20/20（引用不正
0/0/0/5）でした。
