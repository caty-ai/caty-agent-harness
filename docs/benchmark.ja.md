...
- **何が動き、何が動かなかったか。** 両腕とも before ブロックには、最終ブロックま
  ...
  ラス」の上に乗っているのはこのためです。trap クラスだけを見れば C は 8 → 6・B は
...
5. **配線再確認と EV-007b 本走から出た製品への8つの観察** — 主張ではなく、Issue へのポイ
   ...
   はこれを回避するために必要でした）。(e) `scripts/raw-review.sh` の引
   用チェックは、Stop hook の `(日付, task-id)` 接頭辞を残したままの正規
   化に対する前方一致で、200 文字超の引用は棄却し、`blocks > 0 &&
...
14. **EV-007c の見出しを決めたのは trap ではない側の動きです。** 封印された
    ...
    れば C は 8 → 6（Q04 を解き・Q11 は解いてから失い・Q19 を引用で落とした）・
    B は 8 → 8 です。
