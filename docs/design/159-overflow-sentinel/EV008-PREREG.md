...
> また公開にあたり **redact したもの**= アカウント識別子（ドメイン・スロット番号・swap ツール名）・
> 残量の実測値・リセットに紐づくカレンダー日付。**意図的に保持したもの（redact 対象外）**= 実験の
> 再現契約に必要な方法論 — アカウント本数（6）と 4セル+予備2 の割当構造（腕×アカウント交絡の遮断）・
> クォータ余裕率 50% ルール・クォータ窓の種別（7d/5h）とスロット粒度（1日 2〜3・計 3〜7日）。
> redact 箇所は […] で示す。
