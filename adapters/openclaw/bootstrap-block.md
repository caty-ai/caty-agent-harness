...
CATY PAUSE AWARE v1
Before any harness CONSULT, rubric, or CHECKPOINT step, check whether `.caty-agent-harness/DISABLED` exists in the workspace. If it exists, skip all harness instructions in this block and continue with native OpenClaw behavior.
...
The session handoff file must restate the same four restart fields: task id, next action, blockers, and last verified artifact path.
...
Nightly distillation audit is NOT verification: entries it produces are unverified Lessons/Open failures and draft skills only.
