...
parse_flush_file() {
  ...
  awk -v is_today="$is_today" -v torn_today="$torn_today" '
...
if [[ "$pause_state" != enabled ]]; then
  caty_pause_status_record "$workspace" claude-code-flush-intake
  exit 0
fi
...
if [[ ! "$max_fold" =~ ^[0-9]+$ || ! "$lock_attempts" =~ ^[1-9][0-9]*$ ]]; then
  usage
  exit 2
fi
...
if ! take_state_lock "$workspace" claude-code-flush-intake "$lock_attempts" "$lock_sleep"; then
