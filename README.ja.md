...
もう1つの封印レーン **EV-007 / EV-007b** は、主張のもう半分 — 学習ループ — を測りました。結果は**学習イベント 0・本走なし**です: クロスモデル審査が2回 fail-close し、次の計測器を用意した頃にはハーネス + haiku が手持ちのコーパスをすべて 19〜20/20 で解いてしまい、ループが働きかける「繰り返す間違い」の母集団が残っていませんでした。このレーンが残したのは v0.24.0 で出荷した製品変更です（昇格の再発単位が ISO 週から異なるセッション数へ）: [何がどこで止まったか](docs/benchmark.ja.md#ev-007)。
