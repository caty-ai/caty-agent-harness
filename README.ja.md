...
| **発火・結果混合型** — 早期発現の発火・重い全読み込み | gemini-3.7-flash（descriptive — 事前登録 GO 条件の対象外） | 4/4 発火（t31〜82）。bare は L帯で崩壊。分解が L帯1セルの完遂を救済した一方、sentinel セルの 2/4 は正答 0（子ステップの headless permission 停滞） | **記述のみ — 効率主張も default-on 判断もしない。**[セル別の数字](docs/benchmark.ja.md#ev-008) |
