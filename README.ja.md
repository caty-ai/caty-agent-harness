...
|---|---|---|---|
| **全読み込み型** — 水位が単調に増える | claude-haiku-4.5 · claude-sonnet-5 · claude-opus-5 | タスク中盤で発火 → 分解 → 完走。正答維持（sonnet/opus= 発火全セル 20/20・haiku= 19〜20/20） | **効果の主戦場。** 効果は大きい仕事ほど大きい（L帯で最大）。sonnet 中央値 sentinel/bare **0.801**（最良セルでトークン −71%）・opus **0.923**・haiku 0.35（記述的・腕/フェーズ交絡あり — [benchmark](docs/benchmark.ja.md#ev-008) 参照） |
