...
もう1つのレーン **EV-007 / EV-007b** は、主張のもう半分 — 学習ループ — を測りました。結果は**2回とも学習イベント 0**です。EV-007 はクロスモデル審査が fail-close して終了。EV-007b はその後、間違いが実際に再発する trap 注入型の計測器を作り（学習側の腕では v0.24.0 上で毎ラウンド 8/8 の mistake クラスが再発）、その上で封印済みの本走を実行しました — ところが審査は引用を1つ残らず棄却しました。審査側の引用チェックと Stop hook の flush 形式が噛み合っていなかったためで、これがこの走行の製品的な発見です。昇格は 0 件。したがってループが再発する間違いに効くかどうかは、**いまだ未測定**です — 効くとも効かないとも示されていません。このレーンが残したのは v0.24.0 で出荷した製品変更です（昇格の再発単位が ISO 週から異なるセッション数へ）: [何がどこで止まったか](docs/benchmark.ja.md#ev-007)。
