...
**実測 — オーバーフロー・センチネル**（封印付き EV-008・2026-08-25）: センチネル有効時、claude-sonnet-5 は溢れる量の仕事で正答 20/20 を維持したままトークンを削減（sentinel/bare 中央値 0.801・最良セル −71%）。claude-opus-5 は 0.923。検索型ランタイムは実測した全走行で発火ゼロ — それが設計です（Codex 0/127 ターン）。効くモデル・効かせてはいけないモデル → [モデル別プロファイル](#model-effects) ・ [全数字と経緯](docs/benchmark.ja.md#ev-008)。
...
|---|---|---|---|
| **全読み込み型** — 水位が単調に増える | claude-haiku-4.5 · claude-sonnet-5 · claude-opus-5 | タスク中盤で発火 → 分解 → 完走。正答維持（sonnet/opus= 発火全セル 20/20・haiku= 19〜20/20） | **効果の主戦場。** sonnet 中央値 sentinel/bare **0.801**（最良セルでトークン −71%）・opus **0.923**・haiku 0.35（記述的・腕/フェーズ交絡あり — [benchmark](docs/benchmark.ja.md#ev-008) 参照） |
