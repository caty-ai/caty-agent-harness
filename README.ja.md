...
**大きな仕事を段に割り、本当に終わったかを確かめる — そして勝ちも負けも、自分の成績表として公開しているツール。**

**何がどう良くなるのか** — 約束ではなく、実測です:

- **弱いモデルの「できました！」詐称が止まり、仕事が本当に終わるようになります。** 仕事をせずに「done」と申告する率が 98% だった Claude Haiku 4.5（能力低めのモデル）で、虚偽申告は 8% まで下がり — 検証済みの完了は 13% から 43% に上がりました。
- **強いモデルは精度そのまま、大きな仕事でトークンが減ります。** sonnet と opus はハーネス有無でスコア同一 — ただし大きな仕事では、ハーネスなしの走行が 2.40×（sonnet）/ 1.46×（opus）のトークンを使いました。sonnet の小さめの仕事ではハーネスの方が高くつきます — 使いどころは大きな仕事です。さらに窓の約2.4倍の仕事では、ハーネスなしの opus がファイルの 2〜4% しか読まずに提出を繰り返した一方、ハーネスは測定した全セルで全ファイル実読の検証済み完遂に至りました（[両溢れ帯とも 2/2](https://github.com/caty-ai/caty-agent-harness/issues/235)・両腕の予算枠は非同一）。
- **結果は限界も示しています。** 検索型ランタイム（例: Codex）には効果なし — それが設計どおりで、発火したのにハーネスなしより高くついたランタイムも1つあります。sonnet は溢れ帯で 20/20 正答のままファイルの約 10〜12% を読み飛ばし — 検証ゲートは毎回「done」を拒否して、未読ファイルを列挙しました。負けた結果も、勝ちの隣にすべて公開しています。

<sub>封印付き・事前登録・機械採点の4実験ファミリー（2026-08） — すべての数字・方法・弱点は、ハーネスの方が高くつく場面も含めて: [benchmark](docs/benchmark.ja.md) ・ **2分で試す** — [貼り付け1回のプロンプト](#get-started)を、いつもの AI ツールへ。</sub>
