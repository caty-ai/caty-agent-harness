...
もう1つのレーン **EV-007 / EV-007b / EV-007c** は、主張のもう半分 — 学習ループ — を測りました。封印済みの走行は3本です。EV-007 はクロスモデル審査が fail-close して終了。EV-007b はその後、間違いが実際に再発する trap 注入型の計測器を作り（学習側の腕では v0.24.0 上で毎ラウンド 8/8 の mistake クラスが再発）、その上で封印済みの本走を実行しました — ところが審査は引用を1つ残らず棄却しました。審査側の引用チェックと Stop hook の flush 形式が噛み合っていなかったためで、これがその走行の製品的な発見です。ここまで2レーンの昇格は 0 件。**EV-007c は同じ封印系列を、審査が引用できる v0.25.0 の上で再走行しました: 3ターンすべてで昇格が起き（昇格6件・承認ルール2件）、そのうえで封印済みの合格ラインは満たされませんでした** — 学習側の腕の再発率は 0.667、学習なしの腕は 0.727、差分の差分（DiD）は **+0.050** です。ループが学習した唯一のクラス（ファイル内の上書き）はラウンド3で解け、最終ブロックで半分だけ失われました。erratum ファイル型のクラスはどの腕でも動かず、そもそもループが見られる形で書き留められることがありませんでした。これは「1クラスあたり1ビット」の計測器上で **n = 1 の測定が1回** 出たということ — 事前登録されたそのラインの不成立であって、ループ自体の判定ではありません。このレーン群が残したのは v0.24.0 と v0.25.0 で出荷した2つの製品変更です: [何が測れて、何が測れなかったか](docs/benchmark.ja.md#ev-007)。
