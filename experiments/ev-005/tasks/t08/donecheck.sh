...
check_fixed "a01" "adapters/claude-code/INSTALL.md" 'What it does: when the assistant ends a turn in a cwd for a Caty Agent Harness' "Claude install stop-hook description uses the public product name"
...
check_fixed "a05" "adapters/claude-code/checkpoint-stop-hook.sh" '# Caty Agent Harness CHECKPOINT enforcement — Claude Code Stop hook (DESIGN §4.1, Issue #7).' "Claude stop hook banner uses the public product name"
...
check_fixed "a07" "adapters/claude-code/checkpoint-stop-hook.sh" '# Fires when the assistant ends a turn in a Caty Agent Harness workspace that has STATE.md' "Claude stop hook workspace comment uses the public product name"
...
check_fixed "a11" "adapters/claude-code/checkpoint-stop-hook.sh" 'caty-agent-harness CHECKPOINT: workspace files changed after STATE.md was last written' "Claude stop hook reminder uses the public CHECKPOINT prefix"
...
check_fixed "a13" "adapters/claude-code/precompact-flush-hook.sh" '# Caty Agent Harness pre-destruction memory flush — Claude Code PreCompact hook (Issue #47).' "Claude precompact banner uses the public product name"
...
check_fixed "a17" "adapters/claude-code/precompact-flush-hook.sh" 'You are a memory-flush extractor for a Caty Agent Harness workspace about to lose its context window.' "Claude precompact prompt uses the public product name"
...
check_fixed "a19" "adapters/codex/INSTALL.md" 'Codex CLI supports the same lifecycle hooks as Claude Code, so Caty Agent' "Codex install intro uses the public product name"
...
check_fixed "a21" "adapters/codex/INSTALL.md" 'What it does: when the agent ends a turn in a cwd for a Caty Agent Harness' "Codex install stop-hook description uses the public product name"
...
check_fixed "a23" "adapters/codex/checkpoint-stop-hook.sh" '# Caty Agent Harness CHECKPOINT enforcement — Codex CLI Stop hook (DESIGN §4.1, Issue #93).' "Codex stop hook banner uses the public product name"
...
check_fixed "a25" "adapters/codex/checkpoint-stop-hook.sh" '# Fires when the agent ends a turn in a Caty Agent Harness workspace that has STATE.md' "Codex stop hook workspace comment uses the public product name"
...
check_fixed "a29" "adapters/codex/checkpoint-stop-hook.sh" 'caty-agent-harness CHECKPOINT: workspace files changed after STATE.md was last written' "Codex stop hook reminder uses the public CHECKPOINT prefix"
...
check_fixed "a31" "adapters/kimi/INSTALL.md" 'Caty Agent Harness CHECKPOINT enforcement reuses the reference Stop-hook logic' "Kimi install intro uses the public product name"
...
check_fixed "a33" "adapters/kimi/INSTALL.md" 'is about to end a turn in a cwd for a Caty Agent Harness workspace that' "Kimi install stop-hook description uses the public product name"
...
check_fixed "a35" "adapters/kimi/checkpoint-stop-hook.sh" '# Caty Agent Harness CHECKPOINT enforcement — Kimi Code CLI Stop hook (DESIGN §4.1, Issue #93).' "Kimi stop hook banner uses the public product name"
...
check_fixed "a37" "adapters/kimi/checkpoint-stop-hook.sh" '# Fires when the model is about to end a turn in a Caty Agent Harness workspace that has' "Kimi stop hook workspace comment uses the public product name"
...
check_fixed "a41" "adapters/kimi/checkpoint-stop-hook.sh" 'caty-agent-harness CHECKPOINT: workspace files changed after STATE.md was last written' "Kimi stop hook reminder uses the public CHECKPOINT prefix"
...
check_fixed "a53" "templates/updater-cron.tmpl.sh" '# Caty Agent Harness updater cron wrapper template v1' "updater-cron template banner uses the public product name"
