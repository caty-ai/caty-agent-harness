...
**r5 (from the pre-sealing panel, 2026-08-14 — the negative direction):**
1. *Negative validity (mirror of r2)*: for each task, the author constructs a **minimal non-solution probe** — the pre-fix tree plus the smallest edit that satisfies the surface form of the assertions without doing the work — and records either that `donecheck.sh` FAILs on it, or why superficial satisfaction is impossible for this task. Rationale: R11 has no negative direction, and the blindness is arm-asymmetric (W iterates against the visible gate until exit 0, so under-specified assertions convert to verified_pass preferentially in W).
2. *Constant-true declaration is universal (supersedes r4-3's span scoping)*: per-assertion pre-tree status is published from the validity logs for **every** task; every assertion that PASSes on the pre leg carries a one-line reason (invariance guard vs oversight). Measured at adoption: 97/462 assertions constant-true across 19 tasks — the declaration converts a hidden population into an audited one.
3. *No short-circuiting*: a donecheck runs every assertion and reports each `CHECK` line even after failures (R11's verdict is unchanged; the change removes both the missing pre-tree evidence and the unregistered partial-diagnostics property of W's feedback channel). R6's timeout is enforced per-script by the validator (`validate-task.sh` semantics); the per-assertion wording in R6 is corrected to match: the recorded `timeout_s` bounds the whole script run, fail-closed.
4. *Bundled-canonical-source guard (r2 companion)*: shipping fix-tree bytes as an in-replica fixture is admissible **only** when the source criterion itself demands byte-level correspondence to an external canonical artifact (mechanical copy/sync tasks; t03's template is the recorded example). The units.md exemption must say so explicitly; otherwise the fixture route re-introduces the transcription collapse r1 exists to prevent.
5. *Classification pipeline authority (r4-4 clarification)*: R2's nature-based classification at decomposition time is authoritative; r4-4 is a post-mapping audit label. **weakened-MECH** is defined as: a unit classified HUMAN/hybrid at R2 whose extracted behavioral core (R3) is fully mechanized, with the loss recorded. weakened-MECH counts toward MECH share for R3 admissibility; a task whose MECH share consists *only* of weakened-MECH units is flagged for acceptance-seat attention in the footer. **Surface form and audit result (recorded at sealing, r4 — silence is not evidence a rule was applied):** the label is written in the unit tables as **`MECH (weakened)`**; `weakened-MECH` is this document's term for the same class, and a search for the hyphenated form alone will report zero instances (a delta-round seat did exactly that and reported the rule unapplied — it is applied). Bundle-wide census over all 30 admitted tasks (432 units): **MECH 379, HUMAN 32, MOOT 18, MECH (weakened) 3**. All three weakened units are in **t22** — U6 (personal-environment dependencies), U8 (readme-craft process gates), U9 (eleven-item publication gate) — each with its loss recorded in the same row, per the definition above. No task's MECH share consists *only* of weakened units (t22 carries 6 plain MECH alongside its 3), so **no task carries the footer flag**; the flag's absence is an audited outcome, not an unapplied rule.
