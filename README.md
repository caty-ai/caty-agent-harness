...
| **Fires, mixed outcomes** — early-onset firing, heavy full-read | gemini-3.7-flash (descriptive — outside the pre-registered GO conditions) | fires 4/4 (turns 31–82); bare collapses on L-band jobs; decomposition rescued completion in one L cell; 2/4 sentinel cells scored 0 (headless permission stalls in the child steps) | **Descriptive only — no efficiency claim, no default-on judgement.** [Per-cell numbers](docs/benchmark.md#ev-008) |
